...
def main():
    ...
    input_parameters = {
        "RASModel": ["laminar", "kOmegaSST", "v2f", "LaunderSharmaKE"]
    }
...
def create_case_name(parameters_dict):
    p = parameters_dict
    return f"{DATE}_Ampofo_{p['RASModel']}_W{p['x_wall']}_B{p['x_bulk']}_Bts{p['bSource']}"

def run_slurm(cases, n_processors):
    with open(SHARED_SLURM_TEMPLATE) as f:
        lines = f.readlines()
    lines = [l.replace("<JOB-NAME>", f"Ampofo-{DATETIME}") for l in lines]
